...
public final class TrackSwitchInstrumentation extends Instrumentation {
    ...
    private void checkRecoveryReopen(String url) throws Exception {
        ...
        onMain(() -> {
            invoke("startIjkPlayer",new Class<?>[]{Channel.class,String.class,boolean.class,
                            boolean.class,int[].class,long.class},
                    field("activePlayerChannel"),url,false,true,tracks,position);
            for(String name:new String[]{"trackResumePlayer","trackResumePosition","trackResumePlaying"}) {
                ...
                f.set(activity,name.equals("trackResumePlayer")?player():name.equals("trackResumePosition")?0L:true);
            }
        },false);
        waitPrepared();
        SystemClock.sleep(1500);
