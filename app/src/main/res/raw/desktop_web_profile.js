function (config) {
    ...
    get(np, 'platform', function () { return state.config.mac || state.config.ipad ? 'MacIntel' : 'Win32'; });
    get(np, 'vendor', function () { return state.config.ipad ? 'Apple Computer, Inc.' : 'Google Inc.'; });
    get(np, 'maxTouchPoints', function () { return state.config.ipad ? 5 : 0; });
    ...
    // Keep the WebGL identity aligned with the selected browser profile. Sites normally
    // read these values through WEBGL_debug_renderer_info; returning Android's real GPU
    // while claiming Windows/macOS/iPad is both a privacy leak and an inconsistent UI.
    function protectGl(type) {
        if (!type) return;
        var proto = type.prototype, extension = proto.getExtension,
            supported = proto.getSupportedExtensions, parameter = proto.getParameter;
        ...
        proto.getSupportedExtensions = function () {
            ...
            if (!list) return list;
            list = list.slice();
            if (list.map(function (name) { return String(name).toLowerCase(); })
                    .indexOf('webgl_debug_renderer_info') < 0) list.push('WEBGL_debug_renderer_info');
            return list;
        };
        proto.getParameter = function (name) {
            if (name === 37445) return state.config.gpuVendor;
            if (name === 37446) return state.config.gpuRenderer;
            return parameter.apply(this, arguments);
        };
    }
