...
public final class FlyMouseInstrumentation extends Instrumentation {
    ...
    @Override public void onStart() {
        ...
        try {
            ...
            if(args.containsKey("fullscreen")) {
                open(args.getString("fullscreen"),"720p",1f);
                moveTo("full",0.5,0.5); action("click"); SystemClock.sleep(800);
                check(field(source,"fullscreenView")!=null,"Fullscreen not entered");
                check(!json("({editor:document.activeElement.id==='search'})").getBoolean("editor"),
                        "Fullscreen unexpectedly focused input");
                moveTo("seek",0.2,0.5);
                JSONObject hover=json("({moves:mouseMoves,visible:getComputedStyle(document.getElementById('controls')).opacity})");
                check(hover.getInt("moves")>0 && hover.getString("visible").equals("1"),
                        "Fullscreen hover did not reveal controls: "+hover);
                action("down"); dragTo("seek",0.8,0.5,12); action("up"); SystemClock.sleep(200);
                check(json("({value:+document.getElementById('seek').value})").getInt("value")>70,
                        "Fullscreen seek drag failed");
                screenshot("fullscreen-hover.png");
                action("back"); SystemClock.sleep(800);
                check(field(source,"fullscreenView")==null,"Back did not exit fullscreen");
                check(!json("({editor:document.activeElement.id==='search'})").getBoolean("editor"),
                        "Fullscreen exit unexpectedly focused input");
                moveTo("search",0.5,0.5); action("click"); SystemClock.sleep(400);
                check(json("({id:document.activeElement.id})").getString("id").equals("search"),
                        "Normal editor cannot be focused");
                note("PASS fullscreen entry/Back preserve focus; native hover reveals controls; seek drag; normal editor");
            } else if(args.containsKey("zoom")) {
                ...
                // Page zoom uses a different transform from output pixels.
    ...
    private float[] target(String id,double x,double y) throws Exception {
        JSONObject rect=json("(function(){var r=document.getElementById("+JSONObject.quote(id)+").getBoundingClientRect();return {x:r.left+r.width*"+x+",y:r.top+r.height*"+y+",width:innerWidth,height:innerHeight}})()");
        final float[] result=new float[2];
        onMain(() -> {
            View full=(View)field(source,"fullscreenView");
            int[] origin=new int[2],base=new int[2];(full==null?web:full).getLocationOnScreen(origin);((View)field(activity,"root")).getLocationOnScreen(base);
            if(full!=null){
                result[0]=origin[0]-base[0]+(float)rect.getDouble("x")*full.getWidth()/(float)rect.getDouble("width");
                result[1]=origin[1]-base[1]+(float)rect.getDouble("y")*full.getHeight()/(float)rect.getDouble("height");
                return;
            }
