...
for(const movement of [points(80),points(0,80),points(0,0,150)]){
 f=fixture(false);f.fire('touchstart',points());f.fire('touchmove',movement);f.fire('touchend',[],movement);
 assert(f.sent.length>0,'stale webpage state must not suppress a complete two-finger gesture');
}
