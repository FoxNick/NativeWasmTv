...
final class WifiDirectCoordinator implements Closeable {
    ...
    WifiDirectCoordinator(Activity activity, PermissionDelegate permissionDelegate) {
        ...
        WifiP2pManager service = null;
        WifiP2pManager.Channel initializedChannel = null;
        try {
            if (activity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_WIFI_DIRECT)) {
                // Normal permissions can still be missing in repackaged APKs or
                // denied by vendor firmware. They are not runtime permission prompts.
                if (activity.checkCallingOrSelfPermission(Manifest.permission.ACCESS_WIFI_STATE)
                        != PackageManager.PERMISSION_GRANTED
                        || activity.checkCallingOrSelfPermission(Manifest.permission.CHANGE_WIFI_STATE)
                        != PackageManager.PERMISSION_GRANTED) {
                    detail = "Wi-Fi Direct 权限不可用，继续使用局域网";
                } else {
                    service = (WifiP2pManager) activity.getSystemService(Context.WIFI_P2P_SERVICE);
                    if (service != null) {
                        initializedChannel = service.initialize(activity, Looper.getMainLooper(),
                    ...
                    if (initializedChannel == null) {
                        detail = "Wi-Fi Direct 服务不可用，继续使用局域网";
                    }
                }
            }
        } catch (SecurityException error) {
            detail = "Wi-Fi Direct 权限被系统拒绝，继续使用局域网";
            Log.w(TAG, "Wi-Fi Direct initialization denied", error);
        } catch (RuntimeException error) {
            detail = "Wi-Fi Direct 初始化失败，继续使用局域网";
            Log.w(TAG, "Wi-Fi Direct initialization unavailable", error);
        }
        channel = initializedChannel;
        manager = channel != null ? service : null;
        ...
        if (supported) {
            try {
                ...
                supported = false;
                setState(STATE_UNAVAILABLE, "Wi-Fi Direct 初始化失败，继续使用局域网");
                Log.w(TAG, "Wi-Fi Direct receiver unavailable", error);
            }
        }
    }
    ...
    /** Report receiver discovery/group state to the controlling device. */
